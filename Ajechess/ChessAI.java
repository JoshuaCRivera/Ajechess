...
public class ChessAI {    
    ...
    public void playerPawnMoves(Pieces pawn) {        
        ...
        if(location > 80 && forwardPiece == Ajechess.EMPTY && 
                position.board[location-20] == Ajechess.EMPTY && game.safeMove(Ajechess.PLAYER,location,location-20)) {            
    ...
    public void aiPawnMoves(Pieces pawn) {      
        ...
        if(location < 39 && forwardPiece == Ajechess.EMPTY && 
                position.board[location+20] == Ajechess.EMPTY && game.safeMove(Ajechess.COMPUTER,location,location+20)) {            
                positions.add(new Position(position,new Move(location,location+20)));
        }
        
        int rightPiece = position.board[location+11];
        if(rightPiece != Ajechess.ILLEGAL_MOVE) {
            if(rightPiece>0 && rightPiece != Ajechess.EMPTY && game.safeMove(Ajechess.COMPUTER,location,location+11))
                positions.add(new Position(position,new Move(location,location+11))
                		);
        }
        int leftPiece = position.board[location+9];
        if(leftPiece != Ajechess.ILLEGAL_MOVE) {
            if(leftPiece>0 && leftPiece != Ajechess.EMPTY && game.safeMove(Ajechess.COMPUTER,location,location+9))
                positions.add(new Position(position,new Move(location,location+9)));
    ...
    public void playerKnightMoves(Pieces knight) {
        ...
        for(int i=0; i<destinations.length; i++){
            ...
            if(locationDeltas_piece_index != Ajechess.ILLEGAL_MOVE && (locationDeltas_piece_index == Ajechess.EMPTY || locationDeltas_piece_index<0)
    ...
    public void aiKnightMoves(Pieces knight) {
        ...
        for(int i=0; i<destinations.length; i++){
            int locationDeltas_piece_index = position.board[destinations[i]];
            if(locationDeltas_piece_index != Ajechess.ILLEGAL_MOVE && (locationDeltas_piece_index == Ajechess.EMPTY || locationDeltas_piece_index>0) && game.safeMove(Ajechess.COMPUTER,location,destinations[i])) {
                positions.add(new Position(position,new Move(location,destinations[i])));
            }
        }
    }
        ...
        for(int i = 0; i < deltas.length; i++) {
            ...
            while(true) {
                ...
                if(locationDeltas_piece_index == Ajechess.EMPTY || locationDeltas_piece_index < 0) {
                    ...
                    else if(locationDeltas_piece_index != Ajechess.EMPTY) break;
                }
