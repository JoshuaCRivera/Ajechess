...
public class Ajechess extends JFrame implements MouseListener {
                ...
                while(true) {           
                    switch(state) {
                        ...
                        case COMPUTER_MOVE:             
                            ...
                            move = moveSearcher.alphaBeta(COMPUTER, position, 
                                    Integer.MIN_VALUE, Integer.MAX_VALUE, options.levelSlider.getValue()).Move;    
    ...
    public class ChessBoardPane extends JPanel implements MouseListener {     
            ...
            switch(piece_value) {
                case Pieces.PAWN:
                    if(destination == source-10 && squareLocation == EMPTY) 
                    	valid = true;
                    if(destination == source-20 && position.board[source-10] == EMPTY &&
                            squareLocation == EMPTY && source>80) 
                    	valid = true;
                ...
                case Pieces.KING:
                    ...
                    if(piece_value == Pieces.KNIGHT) destinations = new int[] {source-21,source+21,source+19,source-19,                    
        ...
        public void paintComponent(Graphics graphics) {
            ...
            for (int i = 0; i < position.board.length-11; i++)  {
                if (position.board[i] == ILLEGAL_MOVE) continue;                                                                
                int xDirection = i%10;
                ...
                if (position.board[i] == EMPTY) continue;
                if(state == ANIMATING && i==move.source) continue;
        ...
        @Override
        public void mouseClicked(MouseEvent event)  {
            if(state != PLAYER_MOVE) return;
            int location = boardValue(event.getY())*10+boardValue(event.getX());              
            if(position.board[location] == ILLEGAL_MOVE) return;
    ...
    public boolean checkCastling(int destination) {        
        ...
        if(rightRook == null && leftRook == null) 
        	return false;
        if(rightRook != null && rightRook.has_moved && 
                leftRook != null && leftRook.has_moved) 
        	return false;
    ...
    public void animate() {
        if (boardPanel.movingX == boardPanel.desX * 45 && boardPanel.movingY == boardPanel.desY * 45)  {                                           
            ...
            if(source_square>0) {
                ...
                else if(move.destination > 20 && move.destination < 29 && position.playerPieces[source_square].value == Pieces.PAWN) {
                    promotePlayerPawn();  
                }
            }
            else {
    ...
    public void loadPieceImages() {
        ...
        try {
            for(int i=0; i<resource_keys.length; i++) {             
                images.put(images_keys[i],ImageIO.read(resource.getResource((whitePiece?"w":"b")+resource_keys[i])));
                images.put(-images_keys[i],ImageIO.read(resource.getResource((whitePiece?"b":"w")+resource_keys[i])));
            }               
        }catch(IOException exception) {
            exception.printStackTrace();
        }        
    ...
    public void quit() {
        int option = JOptionPane.showConfirmDialog(null,"Are you sure you would like to leave the game?", 
